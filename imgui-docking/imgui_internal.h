...
namespace ImGui
{
    ...
    IMGUI_API static void   NavProcessItems();
    IMGUI_API bool          ItemAdds(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg, ImGuiItemFlags extra_flags);
    ...
    IMGUI_API bool          BeginChildExs(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
    ...
    //IMGUI_API bool          AnimatedTextButton(const char* Icon, const char* label, float& ticks, float& beginticks, int& index, bool& bHovered, std::string& stringbuf, const ImVec2& size_arg);
