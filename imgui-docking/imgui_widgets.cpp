...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    else if (*v)
    {
        ...
        // gradient
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 10.0f));
        ...
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImU32(ImColor(51, 51, 255)), 3.0f, 15);
...
bool ImGui::tab(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(80, 25), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (selected) {
        draw->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImGuiCol_TabLine1), GetColorU32(ImGuiCol_WindowBg), GetColorU32(ImGuiCol_WindowBg), GetColorU32(ImGuiCol_TabLine1));
        ...
        RenderText(ImVec2(bb.Min.x + 15, bb.Min.y + size.y / 2 - label_size.y / 2), label);
        ImGui::PopStyleColor();
    }
    else {
...
bool ImGui::Tab(const char* label, bool selected)
{
    ...
    const ImVec2 label_size = ImGui::CalcTextSize(label, NULL, true);
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(75, 40), 50 + style.FramePadding.x, label_size.y + style.FramePadding.y);
    const ImRect bb(pos, pos + size);
    ImGui::ItemSize(size, style.FramePadding.y);
    if (!ImGui::ItemAdd(bb, id))
        return false;
    ...
    float ANIM_SPEED = 1.00f; // Bigger = Slower
    ...
    auto text_color = ImGui::GetColorU32(ImLerp(ImVec4(180 / 255.f, 180 / 255.f, 180 / 255.f, 180 / 255.f), ImVec4(112 / 255.f, 146 / 255.f, 190 / 255.f, 255.f / 255.f), t));
    //filled_rect2(ImVec2{ pos.x - 15.f , pos.y }, ImVec2{ 30 * (bb.Min.x + 100, bb.Max.y - 2), 5 }, ImColor(96, 0, 199, a), false);
    draw->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 2), bb.Max, ImColor(51, 51, 255, a));
    draw->AddText(ImVec2(pos.x + 75 / 2 - label_size.x / 2, pos.y + 28 - label_size.y / 2), ImColor(255, 255, 255), label);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags);
    return pressed;
}
