...
void Script::ComboBox() {
    ...
    if (config.Includes.selectedItemWP == 0) {
        ...
        config.Includes.Active_Weapon = "CUSTOM";
        ...
        config.Includes.Active_Weapon = "THOMPSON";
        ...
        config.Includes.Active_Weapon = "SEMI";
...
void Script::pQuerySleep(int ns) {
    LONGLONG NtDelayExecution;
    LONGLONG wanted_time;
    LONGLONG current_time;

    QueryPerformanceFrequency((LARGE_INTEGER*)&NtDelayExecution);
    NtDelayExecution /= 1000;

    QueryPerformanceCounter((LARGE_INTEGER*)&current_time);

    wanted_time = current_time / NtDelayExecution + ns;
    current_time = 0;

    while (current_time < wanted_time) {
        QueryPerformanceCounter((LARGE_INTEGER*)&current_time);
        current_time /= NtDelayExecution;
    }
}



void Script::OverlayStuff()
{
    
    //if (GetAsyncKeyState(config.Includes.hpbind) & 1)
    //{
    //    config.Includes.hipfire = !config.Includes.hipfire;
    //}
    //if (GetAsyncKeyState(config.Includes.hsbind) & 1)
    //{
    //    config.Includes.showing = !config.Includes.showing;
    //}
    if (menu.Includes.show_info == true)
    {
        
        ImGui::SetNextWindowSize(ImVec2(155, 95));
        DWORD flags = ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoDecoration;
        ImGui::Begin("Dopeware.cc", &menu.Includes.show_info, flags);
        {
            ImGui::SetCursorPos(ImVec2(6, 4));
            ImGui::BeginChildss("##StatInfo", ImVec2(143.5, 85));
            if (config.Includes.recoil == true)
            {
                ImGui::SetCursorPos(ImVec2(8, 2));
                ImGui::Text("Script:");
                ImGui::SameLine(47);
                ImGui::TextColored(ImColor(config.Includes.R, config.Includes.G, config.Includes.B), "ENABLED");
            }
            else {
                ImGui::SetCursorPos(ImVec2(8, 2));
                ImGui::Text("Script:");
                ImGui::SameLine(47);
                ImGui::TextColored(ImColor(config.Includes.R, config.Includes.G, config.Includes.B), "DISABLED");
            }
            if (config.Includes.Active_Weapon == "NO WEAPON")
            {
                ImGui::SetCursorPos(ImVec2(8, 24));
                ImGui::Text("Weapon:");
                ImGui::SameLine(62);
                ImGui::TextColored(ImColor(config.Includes.R, config.Includes.G, config.Includes.B), config.Includes.Active_Weapon);
            }
            else {
                ImGui::SetCursorPos(ImVec2(8, 24));
                ImGui::Text("Weapon:");
            ...
            if (config.Includes.Active_Scopes == "NO SCOPE")
            {
                ImGui::SetCursorPos(ImVec2(8, 46));
                ImGui::Text("Scope:");
                ImGui::SameLine(48);
                ImGui::TextColored(ImColor(config.Includes.R, config.Includes.G, config.Includes.B), config.Includes.Active_Scopes);
            }
            else {
                ImGui::SetCursorPos(ImVec2(8, 46));
                ImGui::Text("Scope:");
            ...
            if (config.Includes.Active_Barrel == "NO BARREL")
            {
                ImGui::SetCursorPos(ImVec2(8, 68));
                ImGui::Text("Barrel:");
                ImGui::SameLine(48);
                ImGui::TextColored(ImColor(config.Includes.R, config.Includes.G, config.Includes.B), config.Includes.Active_Barrel);
            }
            else {
                ImGui::SetCursorPos(ImVec2(8, 68));
                ImGui::Text("Barrel:");
...
void Script::ScopeNone(bool S) {
    if (S == true) {
        ...
        config.Includes.Active_Scopes = "No Scope";
    }
}
...
void Script::BarrelNone(bool B) {
    if (B == true) {
        ...
        config.Includes.Active_Barrel = "No Barrel";
    }
}
...
void Script::keybinds() {
    ...
    if (GetAsyncKeyState(config.Includes.lrbind) & 1) {
        ...
        else if (config.Includes.lr_active == false) {
            WeaponNone(true);
        }
    }
    if (GetAsyncKeyState(config.Includes.mpbind) & 1) {
        ...
        else if (config.Includes.mp5_active == false) {
            WeaponNone(true);
        }
    }
    if (GetAsyncKeyState(config.Includes.csbind) & 1) {
        ...
        else if (config.Includes.custom_active == false) {
            WeaponNone(true);
        }
    }
    if (GetAsyncKeyState(config.Includes.tmbind) & 1) {
        ...
        else if (config.Includes.thompson_active == false) {
            WeaponNone(true);
        }
    }
    if (GetAsyncKeyState(config.Includes.smbind) & 1) {
        ...
        else if (config.Includes.semi_active == false) {
            WeaponNone(true);
        }
    }
    if (GetAsyncKeyState(config.Includes.m2bind) & 1) {
        ...
        else if (config.Includes.m249_active == false) {
            WeaponNone(true);
        }
    }
    //Scopes
    ...
    if (GetAsyncKeyState(config.Includes.mbbind) & 1) {
        ...
        else if (config.Includes.boost_active == false) {
            BarrelNone(true);
        }
    }
    if (GetAsyncKeyState(config.Includes.bbind) & 1) {
        ...
        else if (config.Includes.break_active == false) {
            BarrelNone(true);
        }
    }
...
double CurrentWeapon::x(int b) {
    if (config.Includes.randomxy_active == false) {
        if (config.Includes.ak_active == true) return (RecoilTables::Recoil_Ak47[b].x * ScopeAttachment() * BarrelAttachment() / (-0.0297555555 * (config.Includes.game_sensitivity) * 3.0 * (config.Includes.game_fov / 100.0))) / config.Includes.XCONTROL;
        if (config.Includes.lr_active == true) return (RecoilTables::Recoil_Lr300[b].x * ScopeAttachment() * BarrelAttachment() / (-0.0295 * (config.Includes.game_sensitivity) * 3.0 * (config.Includes.game_fov / 100.0))) / config.Includes.XCONTROL;
    ...
    if (config.Includes.randomxy_active == true) {                                                                 //X RANDOMIZATION ON AND OFF 
        if (config.Includes.ak_active == true) return ((RecoilTables::Recoil_Ak47[b].x * ScopeAttachment()) * BarrelAttachment()) / (-0.0297555555 * (config.Includes.game_sensitivity) * 3.0 * (config.Includes.game_fov / 100.0)) / config.Includes.XCONTROL + Randomization(config.Includes.XSCALE + config.Includes.XSCALE) + (-config.Includes.XSCALE);
...
double CurrentWeapon::y(int b) {                                                                                       //Y RANDOMIZATION ON AND OFF
    if (config.Includes.randomxy_active == false) {
        if (config.Includes.ak_active == true) return (RecoilTables::Recoil_Ak47[b].y * ScopeAttachment() * BarrelAttachment() / (-0.0297555555 * (config.Includes.game_sensitivity) * 3.0 * (config.Includes.game_fov / 100.0))) / config.Includes.YCONTROL;
    ...
    if (config.Includes.randomxy_active == true) {
        if (config.Includes.ak_active == true) return ((RecoilTables::Recoil_Ak47[b].y * ScopeAttachment()) * BarrelAttachment()) / (-0.0297555555 * (config.Includes.game_sensitivity) * 3.0 * (config.Includes.game_fov / 100.0)) / config.Includes.YCONTROL + Randomization(config.Includes.YSCALE + config.Includes.YSCALE) + (-config.Includes.YSCALE);
